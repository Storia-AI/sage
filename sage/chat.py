...
def main():
    ...
    parser.add("--share", default=False, help="Make the Gradio app publicly accessible.")
    ...
    def source_md(file_path: str, url: str) -> str:
        """Format a context source in Markdown."""
        return f"[{file_path}]({url})"

    async def _predict(message, history):
        """Perform one RAG operation."""
        history_langchain_format = [
            (HumanMessage(content=human), AIMessage(content=ai)) for human, ai in history
